...
_HIGH_STAGE_BLOCK_TRIM_INTERVAL = 4
...
def make_vdn_forward(attn, state, block_index):
    ...
    def vdn_forward(x, rope_freqs=None, transformer_options=None):
        ...
        if _high_stage_block_trim_due(transformer_options, block_index):
            _maybe_trim_high_allocator(
                state,
                transformer_options,
                location=f"attention_block:{block_index}",
            )
        return out
...
def _high_stage_block_trim_due(transformer_options, block_index):
    return bool(
        isinstance(transformer_options, dict)
        and transformer_options.get(FLOW_STAGE_KEY) == "high"
        and (int(block_index) + 1) % _HIGH_STAGE_BLOCK_TRIM_INTERVAL == 0
    )


def _maybe_trim_high_allocator(state, transformer_options, *, location="model_return"):
    if (
    ...
    _log.info(
        "[vdn] high-stage allocator pressure trim location=%s allocator_before=%s allocator_after=%s",
        location,
        before,
        after,
    )
